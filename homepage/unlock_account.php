...
/**
 * unlock_account.php
 * ClassInstruct — Unlock Account via Email Link + New Password
 *
 * Flow (mirrors forgot_password.php exactly, but also clears lockout on success):
 *   GET  (no token)         → show "send unlock link" form
 *   POST ?step=send         → generate token, email it, show confirmation
 *   GET  ?token=...         → validate token, show new-password form
 *   POST ?step=reset&token= → validate token + new password, unlock + update DB
 *
 * Token rules:
 *   - 64-char hex (32 random bytes)
 *   - Stored as SHA-256 hash in unlock_token column
 *   - Expires after 30 minutes (unlock_token_exp)
 *   - Single-use: cleared on successful password change
 *
 * On success the account is fully unlocked:
 *   login_attempts = 0, lockout_until = NULL, perm_locked = 0
 */
...
if (!empty($_GET['token']) && $_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if (!preg_match('/^[0-9a-f]{64}$/', $rawToken)) {
        $step      = 'error';
        $pageError = 'Invalid or malformed unlock link.';
    } else {
        ...
        if (!$user) {
            $step      = 'error';
            $pageError = 'This unlock link is invalid or has already been used.';
            ...
            $expiredStmt->execute();
            $step      = 'error';
            $pageError = 'This unlock link has expired. Please request a new one.';
        } else {
            $step       = 'reset_form';
            $resetToken = $rawToken;
        }
...
// ══════════════════════════════════════════════════════════════════════════════
// STEP B — POST: send unlock email OR process new password
// ══════════════════════════════════════════════════════════════════════════════
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $postStep = trim($_POST['step'] ?? 'send');

    // ── B1: Send the unlock email ─────────────────────────────────────────────
    if ($postStep === 'send') {
        ...
        if (empty($email) || !filter_var($email, FILTER_VALIDATE_EMAIL)) {
            ...
            $stmt = $conn->prepare(
                'SELECT id, first_name, google_id FROM users WHERE email = ? LIMIT 1'
            );
            ...
            if ($user) {
                // Google-only accounts — send a notice instead
                if (!empty($user['google_id'])) {
                    ...
                    try {
                        ...
                        $mail->isHTML(true);
                        $mail->Subject = 'ClassInstruct — Account unlock request';
                        $mail->Body    = buildGoogleNoticeEmail($firstName, $email);
                        $mail->AltBody = "Hi {$firstName},\n\nYour ClassInstruct account uses Google Sign-In. Please use 'Continue with Google' on the sign-in page.";
                        $mail->send();
                    } catch (MailException) { /* silent */ }
                } else {
                    // Normal account: generate unlock token
                    ...
                    $unlockUrl = $appUrl . '/unlock_account.php?token=' . $rawToken;
                    ...
                    try {
                        ...
                        $mail->addAddress($email, $firstName);
                        $mail->isHTML(true);
                        $mail->Subject = 'Unlock your ClassInstruct account';
                        $mail->Body    = buildUnlockEmail($firstName, $unlockUrl, $email);
                        $mail->AltBody = "Hi {$firstName},\n\nUnlock your account and set a new password at:\n{$unlockUrl}\n\nExpires in 30 minutes. If you didn't request this, ignore this email.";
                        $mail->send();
                    } catch (MailException $e) {
                        error_log('[ClassInstruct] Unlock mailer error: ' . $mail->ErrorInfo);
                    }
                }
            }
        }
    ...
    // ── B2: Process new password + unlock account ─────────────────────────────
    if ($postStep === 'reset') {
        ...
        if (!preg_match('/^[0-9a-f]{64}$/', $rawToken)) {
            $step      = 'error';
            $pageError = 'Invalid unlock token.';
            ...
            $stmt = $conn->prepare(
                'SELECT id, email, unlock_token_exp FROM users WHERE unlock_token = ? LIMIT 1'
            );
            $stmt->bind_param('s', $tokenHash);
            $stmt->execute();
            $user = $stmt->get_result()->fetch_assoc();
...
// ─── Email builders ──────────────────────────────────────────────────────────
function buildUnlockEmail(string $firstName, string $unlockUrl, string $email): string
{
    return <<<HTML
        ...
        <tr><td align="center">
          <table width="480" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:16px;box-shadow:0 4px 24px rgba(0,0,0,0.08);overflow:hidden;max-width:480px;width:100%;">
            ...
            <tr><td style="padding:40px 40px 32px;">
              ...
              <h1 style="font-size:22px;font-weight:700;color:#111827;margin:0 0 8px;text-align:center;">Unlock Your Account</h1>
              <p style="font-size:14px;color:#6b7280;margin:0 0 24px;line-height:1.6;text-align:center;">
                Hi <strong>{$firstName}</strong>, click the button below to unlock your account and set a new password.<br>
                This link expires in <strong>30 minutes</strong>.
              </p>
              <div style="text-align:center;margin:0 0 28px;">
                <a href="{$unlockUrl}" style="display:inline-block;padding:14px 32px;background:#6c63ff;color:#fff;text-decoration:none;border-radius:10px;font-size:15px;font-weight:600;">
                  Unlock My Account
                </a>
              </div>
...
<head>
  ...
  <title>Unlock Account — ClassInstruct</title>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* Unlock accent — red→orange→purple to signal security alert */
    .card-accent {
      height: 3px;
      background: linear-gradient(90deg, #ef4444, #f97316, #6c63ff);
    }
    ...
    .lock-notice {
      ...
      font-size: .8125rem; color: #c2410c; margin-bottom: 22px;
      line-height: 1.5;
    }
    .lock-notice i { margin-top: 1px; flex-shrink: 0; }
    ...
    .state-box .btn-primary { max-width: 260px; margin: 0 auto; }
...
<div class="card">
  ...
  <div class="card-body">
    ...
    <?php if ($step === 'form'): ?>
      ...
      <p class="subtitle">Too many failed sign-in attempts. Enter your email and we'll send a secure link to unlock your account.</p>

      <div class="lock-notice">
        <i class="fa-solid fa-triangle-exclamation"></i>
        <span>Your account has been temporarily locked for security. Unlocking will let you set a new password.</span>
      </div>
      ...
      <form method="POST" action="unlock_account.php" novalidate id="unlockForm">
        ...
        <button type="submit" class="btn-primary" id="submitBtn">
          ...
          <span class="btn-label">Send Unlock Link</span>
        </button>
      </form>
    ...
    <?php elseif ($step === 'sent'): ?>
      <div class="state-box">
        ...
        <p>If a matching account exists, we've sent an unlock link to your inbox.<br>The link expires in <strong>30 minutes</strong>.</p>
    ...
    <?php elseif ($step === 'reset_form'): ?>
      ...
      <p class="subtitle">Choose a strong password. Your account will be unlocked immediately after.</p>

      <?php if ($pageError): ?>
        <div class="alert-error"><i class="fa-solid fa-circle-exclamation"></i><?= htmlspecialchars($pageError) ?></div>
      <?php endif; ?>

      <form method="POST" action="unlock_account.php" novalidate id="resetForm">
        ...
        <div class="form-group">
          ...
          <div class="pw-wrap">
            <input type="password" id="pwInput" name="password"
              ...
              style="padding-left:.875rem;" required>
        ...
        <div class="form-group">
          ...
          <div class="pw-wrap">
            <input type="password" id="pwConfirmInput" name="password_confirm"
        ...
        <button type="submit" class="btn-primary" id="resetBtn">
          ...
          <span class="btn-label">Save Password &amp; Unlock Account</span>
        </button>
      </form>
    ...
    <?php elseif ($step === 'error'): ?>
      <div class="state-box">
        ...
        <a href="unlock_account.php" class="btn-primary" style="text-decoration:none;">
